...
require __DIR__ . '/include/dump.inc.php';
...
$type = reg_glob('type');
$dumpwindow = $_GET['dumpwindow'] ?? '';
$navbar = $_GET['navbar'] ?? '';
$step = $_GET['step'] ?? '';
$options = $_GET['options'] ?? '';
$nbRecords = $_GET['nbRecords'] ?? '';
$feedback = '';
$complete_insert = 0;
$drop_table = 0;
...
if ($popup_DB->Infos['Version'] >= 32303) {
    ...
    while ($list = $popup_DB->next_record()) {
        ...
        [$tbl_list_records[]] = $popup_DB2->next_record();
    }
}
    ...
    echo '<HTML>
         ...
         . 'px}
			...
			function refresh_main() {
				parent.opener.location = "main.php?db='
         . urlencode($db)
         . '&tbl='
         . urlencode($tbl_js)
         . '";
...
if (1 == $dumpwindow) {
    ...
    if (count($tbl_list) > 1) {
            ...
            if ($already_done < $rec_2_do) {
                ...
                if ($tmp_offset < ($current_tbl_records + $past_tbl)) {
                    ...
                    if (($current_tbl_records + $past_tbl) > ($tmp_offset + $split)) {
                        ...
                        $feedback .= do_data_dump($popup_DB, $tbl_list_name[$tbl_list[$i]], $complete_insert, ($tmp_offset - $past_tbl), $split);

                        $already_done += $split;
                    } else {
                        ### NO

                        $feedback .= do_data_dump($popup_DB, $tbl_list_name[$tbl_list[$i]], $complete_insert, ($tmp_offset - $past_tbl), (($current_tbl_records + $past_tbl) - $tmp_offset));
        ...
        $feedback = do_data_dump($popup_DB, $tbl, $complete_insert, $offset, $split);
