...
function show_tbl_indexes($sql_ref, $tbl)
{
    ...
    if ($sql_ref->num_rows()) {
        while ($ret = $sql_ref->next_record()) {
            ...
            $non_unique = $ret['Non_unique'] ?? '';

            $key_name = $ret['Key_name'] ?? '';

            $seq_in_index = $ret['Seq_in_index'] ?? '';

            $column_name = $ret['Column_name'] ?? '';

            $collation = $ret['Collation'] ?? '';

            $cardinality = $ret['Cardinality'] ?? '';

            $sub_part = $ret['Sub_part'] ?? '';
...
function set_tbl_comment($sql_ref, $tbl)
{
    ...
    $do_comment = $_POST['do_comment'] ?? '';

    $comment = $_POST['comment'] ?? '';

    $comment = addslashes(magic_quote_bypass($comment));

    $comment = htmlentities($comment, ENT_QUOTES | ENT_HTML5);
...
function insert_into_tbl($sql_ref, $tbl)
{
    ...
    if (1 == $add) {
        ...
        while ($arrDesc = $sql_ref->next_record()) {
            ...
            if (isset($_POST["field_value_$nbFields"]) && is_array($_POST["field_value_$nbFields"])) {
                $value = '';

                $tmp_value = $_POST["field_value_$nbFields"] ?? '';

                for ($i = 0, $iMax = count($tmp_value); $i < $iMax; $i++) {
                ...
                $value = $_POST["field_value_$nbFields"] ?? '';
            }

            $function = $_POST["field_function_$nbFields"] ?? '';

            $fields_list .= sql_back_ticks($field, $sql_ref) . ',';

            if (isset($_POST["field_null_$nbFields"])) {
                $values_list .= 'NULL, ';
            } else {
                if (0 == $function) {
        ...
        $fields_list = mb_substr($fields_list, 0, -1);

        $values_list = mb_substr($values_list, 0, -2);
        ...
        while ($arrDesc = $sql_ref->next_record()) {
            ...
            $default = isset($arrDesc['Default']) ? htmlentities($arrDesc['Default'], ENT_QUOTES | ENT_HTML5) : '';
...
function view_tbl($sql_ref, $tbl, $extra_query = '')
{
    ...
    $order_field = isset($_POST['order_field']) ? magic_quote_bypass($_POST['order_field']) : '';
    ...
    $navSqlOrderBy = isset($_POST['navSqlOrderBy']) ? magic_quote_bypass($_POST['navSqlOrderBy']) : '';
    ...
    $output_js .= 'function field_chg(field) {
                       ...
                       function full_text() {
                       		document.navFrm.dont_reduce_blob.value = ' . ((0 == $dont_reduce_blob) ? 1 : 0) . ';
    ...
    $output = $txt['display_records'] . ' ' . mb_strtolower($txt['from']) . ' ' . ($limit + 1) . ' ' . mb_strtolower($txt['to']) . ' ';

    $output .= (($limit + $nb2show) > $nbEnreg) ? $nbEnreg : ($limit + $nb2show);

    $output .= ' (' . $nb2show . ' ' . mb_strtolower($txt['records']) . ')';
        ...
        for ($i = 0, $iMax = count($arrView); $i < $iMax; $i++) {
            ...
            if ('' == $arrView[$i] && isset($arrView[$i])) {
                ...
                if ((isset($blob_array[$i])) && $conf['reduceBlob'] && !$dont_reduce_blob && (mb_strlen($arrView[$i]) > $conf['reduceBlob'])) {
                    $output .= '<td><font ' . $font . '>' . htmlentities(mb_substr($arrView[$i], 0, $conf['reduceBlob']), ENT_QUOTES | ENT_HTML5) . '...</font></td>';
                } else {
                    if (!isset($arrView[$i])) {
...
function rename_tbl($sql_ref, $tbl)
{
    ...
    $new_tbl = isset($_POST['new_tbl']) ? magic_quote_bypass($_POST['new_tbl']) : '';

    $input_error = (eregi('"', $new_tbl) || !mb_strlen($new_tbl) || eregi('[\]', $new_tbl) || eregi('<', $new_tbl) || eregi('>', $new_tbl));

    if ($input_error && 1 == $rename) {
...
function copy_tbl($sql_ref, $tbl)
{
    ...
    $copy_db_dest = isset($_POST['copy_db_dest']) ? magic_quote_bypass($_POST['copy_db_dest']) : '';

    $copy_tbl_dest = isset($_POST['copy_tbl_dest']) ? magic_quote_bypass($_POST['copy_tbl_dest']) : '';

    $input_error = (eregi('"', $copy_tbl_dest) || !mb_strlen($copy_tbl_dest) || eregi('[\]', $copy_tbl_dest) || eregi('<', $copy_tbl_dest) || eregi('>', $copy_tbl_dest));

    if ($input_error && 1 == $copy) {
        $output = '<FONT color=#FF0000><B>' . $txt['invalid_dest'] . '</B></FONT>';
...
function move_tbl($sql_ref, $tbl)
{
    ...
    $move_db_dest = isset($_POST['move_db_dest']) ? magic_quote_bypass($_POST['move_db_dest']) : '';

    $move_tbl_dest = isset($_POST['move_tbl_dest']) ? magic_quote_bypass($_POST['move_tbl_dest']) : '';

    $input_error = (eregi('"', $move_tbl_dest) || !mb_strlen($move_tbl_dest) || eregi('[\]', $move_tbl_dest) || eregi('<', $move_tbl_dest) || eregi('>', $move_tbl_dest));

    if ($input_error && 1 == $move) {
        $output = '<FONT color=#FF0000><B>' . $txt['invalid_dest'] . '</B></FONT>';
...
function split_type($type)
{
    if (preg_match('\(', $type)) {
        ...
        $fparenthesis = mb_strpos($type, '(');

        $lparenthesis = mb_strpos($type, ')', $fparenthesis);

        $mytab[0] = mb_substr($type, 0, $fparenthesis);

        $mytab[1] = mb_substr($type, $fparenthesis + 1, $lparenthesis - $fparenthesis - 1);
...
function modif_field($sql_ref, $tbl)
{
    ...
    while (list(, $non_unique, $key_name, , $column_name) = $sql_ref->next_record()) {
        $keys[$column_name]['non_unique'] = $keys[$column_name]['non_unique'] ?? '';
        ...
        if (0 == $non_unique && (0 == $keys[$column_name]['non_unique'] || '' == $keys[$column_name]['non_unique'])) {
    ...
    if (1 == $modify && !$input_error) {
        ...
        for ($i = 0; $i < $nb_fields; $i++) {
            $old_field_name = isset($_POST["old_field_name_$i"]) ? magic_quote_bypass($_POST["old_field_name_$i"]) : '';

            $field_name = isset($_POST["field_name_$i"]) ? magic_quote_bypass($_POST["field_name_$i"]) : '';

            $field_type = isset($_POST["field_type_$i"]) ? $data_types[$_POST["field_type_$i"]] : '';

            $field_length = ('' == $_POST["field_length_$i"]) ? '' : '(' . magic_quote_bypass($_POST["field_length_$i"]) . ')';

            $field_attribute = $data_attributes[$_POST["field_attribute_$i"]];

            $field_null = ('yes' == $_POST["field_null_$i"]) ? '' : 'NOT NULL';

            $field_default = ('' == $_POST["field_default_$i"]) ? '' : 'DEFAULT \'' . addslashes(magic_quote_bypass($_POST["field_default_$i"])) . '\'';

            $field_increment = ('yes' == $_POST["field_increment_$i"]) ? 'AUTO_INCREMENT' : '';

            $field_unique = isset($_POST["field_unique_$i"]) ? 1 : 0;

            $field_index = isset($_POST["field_index_$i"]) ? 1 : 0;

            $field_primary = isset($_POST["field_primary_$i"]) ? 1 : 0;

            $field_fulltext = isset($_POST["field_fulltext_$i"]) ? 1 : 0;
        ...
        for ($i = 0, $iMax = count($add_unique); $i < $iMax; $i++) {
            $sql_add_unique .= ' ADD UNIQUE (' . sql_back_ticks($add_unique[$i], $sql_ref) . "),\n";
        }

        ### Index keys

        for ($i = 0, $iMax = count($add_index); $i < $iMax; $i++) {
            $sql_add_index .= ' ADD INDEX (' . sql_back_ticks($add_index[$i], $sql_ref) . "),\n";
        }

        ### Primary keys

        for ($i = 0, $iMax = count($add_primary); $i < $iMax; $i++) {
            $sql_primary_keys = '';

            if (0 == $i && 1 == $primary_key_defined) {
                $sql_add_primary .= ' DROP PRIMARY KEY, ';

                for ($j = 0, $jMax = count($primary_key_name); $j < $jMax; $j++) {
        ...
        for ($i = 0, $iMax = count($add_fulltext); $i < $iMax; $i++) {
        ...
        $total_sql = mb_substr($total_sql, 0, -2);
        ...
        if ($input_error) {
            ...
            $to_modify = $_POST['field_check'] ?? [];

            ### No fields specified, going back

            if (0 == count($to_modify)) {
        ...
        while ($tmp_value = $sql_ref->next_record()) {
            if (isset($tmp_value['Comment']) && 'FULLTEXT' == $tmp_value['Comment']) {
        ...
        while (list($field, $type, $null_not_allowed, $key, $default, $extra) = $sql_ref->next_record()) {
            if (isset($to_modify[$i])) {
                ...
                $key_check_unique = $keys[$field]['non_unique'] ?? '';

                $key_check_index = $keys[$field]['key_name'] ?? '';

                $key_check_primary = $keys[$field]['primary'] ?? '';

                $key_check_fulltext = (isset($full_text_keys[$field]) && 1 == $full_text_keys[$field]) ? 1 : 0;

                ### Add ' ' around the 0 to avoid null && 0 confusion

                if ('0' == $key_check_unique) {
                ...
                if (1 == $key_check_primary) {
                    $check_primary = 'CHECKED DISABLED';
                }

                if (1 == $key_check_fulltext) {
                    $check_fulltext = 'CHECKED DISABLED';
                }

                $type = split_type($type);

                $field_type = mb_strtoupper($type[0]);

                $field_length = $type[1];

                if ('ENUM' == $field_type || 'SET' == $field_type) {
                ...
                $field_attribute = mb_strtoupper($type[2]);

                $null_allowed = ('YES' == $null_not_allowed) ? 'SELECTED' : '';

                $null_not_allowed = ('' == $null_not_allowed) ? 'SELECTED' : '';

                $auto_increment = ('auto_increment' == $extra) ? 'SELECTED' : '';
                ...
                $output .= '<tr>
								...
								<tr>	
									...
									<td><input type=text name="field_default_' . $valide . '" class="trous" value="' . htmlentities($default, ENT_QUOTES | ENT_HTML5) . '"></td>
									<td colspan="2">
...
function drop_field($sql_ref, $tbl)
{
    ...
    if (1 == $drop_field) {
        ...
        for ($i = 0, $iMax = count($field_list); $i < $iMax; $i++) {
        ...
        while (list($field) = $sql_ref->next_record()) {
            if (isset($to_modify[$i])) {
                $field = htmlentities(magic_quote_bypass($field), ENT_QUOTES | ENT_HTML5);
        ...
        if (0 != $valide) {
            $field_list = mb_substr($field_list, 0, -1);
...
{
    global $colors, $font, $_POST, $data_attributes, $data_types, $txt;

    $add_field = $_POST['add_field'] ?? '';

    $enter_fields = $_POST['enter_fields'] ?? '';
    ...
    if (1 == $enter_fields && '' == $nb_fields) {
        $enter_fields = 0;
    }

    if (1 == $add_field) {
        for ($i = 0; $i < $nb_fields; $i++) {
            $name = magic_quote_bypass($_POST["field_name_$i"]);

            $input_error = $input_error || (eregi('"', $name) || !mb_strlen($name) || eregi('[\]', $name) || eregi('<', $name) || eregi('>', $name));
        }
    }

    if (1 == $add_field && !$input_error) {
        ...
        for ($i = 0; $i < $nb_fields; $i++) {
            ...
            $fulltext = $_POST["field_fulltext_$i"] ?? '';
            ...
            if ('first pos' == $field_pos) {
                $sql_query .= ' FIRST';

                $field_pos = $name;
            } elseif ('end pos' != $field_pos) {
        ...
        $output .= '<BR>' . return_2_table($sql_ref->Database, $tbl);
    } elseif (1 == $enter_fields) {
        if ($input_error && 1 == $add_field) {
...
function drop_key($sql_ref, $tbl)
{
    ...
    $confirm_drop_key = $_POST['confirm_drop_key'] ?? '';

    $output = '';

    if (1 == $confirm_drop_key) {
        if ('PRIMARY' == $_POST['key']) {
            $sql_query = 'ALTER TABLE ' . sql_back_ticks($tbl, $sql_ref) . ' DROP PRIMARY KEY';
        } else {
            $sql_query = 'ALTER TABLE ' . sql_back_ticks($tbl, $sql_ref) . ' DROP INDEX ' . sql_back_ticks($_POST['key'], $sql_ref);
        }
        ...
        $output .= '<BR><BR>' . $txt['the_key'] . ' ' . $_POST['key'] . ' ' . $txt['been_deleted'] . '.';

        $output .= return_2_table($sql_ref->Database, $tbl);
    } else {
        $output = $txt['confirm_key'] . ' :<BR><B>' . $_POST['key'] . '</B> ?';

        $output .= '<form action="main.php?db=' . urlencode($sql_ref->Database) . '&tbl=' . urlencode($tbl) . '&action=drop_key" method="post">';

        $output .= '<input type=submit value="' . $txt['Yes'] . '" class="bosses">&nbsp;<input type=button value="' . $txt['No'] . '" onClick="javascript:history.go(-1);" class="bosses">';

        $output .= '<input type="hidden" name="key" value="' . $_POST['key'] . '">';
...
function change_tbl_type($sql_ref, $tbl)
{
    ...
    $new_tbl_type = $_POST['new_tbl_type'] ?? '';
    ...
    if (1 == $change_type) {
        ...
        for ($i = 0, $iMax = count($avl_type); $i < $iMax; $i++) {
            if (mb_strtolower($tbl_type) == $avl_type[$i]) {
                $selected = 'SELECTED';
            } else {
                $selected = '';
            }
...
function modif_record($sql_ref, $tbl)
{
    ...
    global $_GET, $HTTP_COOKIE_VARS;

    global $data_functions;

    $modif_record = $_POST['modif_record'] ?? '';

    $query = isset($_GET['query']) ? magic_quote_bypass($_GET['query']) : '';
    ...
    if (1 == $modif_record) {
        ...
        for ($i = 0; $i <= $nb_fields; $i++) {
            $field_name = magic_quote_bypass($_POST["field_name_$i"]);

            $field_value = isset($_POST["field_value_$i"]) ? magic_quote_bypass($_POST["field_value_$i"]) : '';

            $field_function = $_POST["field_function_$i"] ?? '';

            $field_is_null = $_POST["null_value_$i"] ?? '';
            ...
            if ($field_is_null) {
                ...
                if (is_array($_POST["field_value_$i"])) {
                    $value = '';

                    $tmp_value = $_POST["field_value_$i"] ?? '';

                    for ($j = 0, $jMax = count($tmp_value); $j < $jMax; $j++) {
        ...
        $serial = isset($HTTP_COOKIE_VARS['serial']) ? unserialize(magic_quote_bypass($HTTP_COOKIE_VARS['serial'])) : [];
...
{
    global $font, $colors, $_POST, $conf, $txt;
    ...
    if (1 == $do_select) {
        ...
        for ($i = 0; $i < $nb_fields; $i++) {
            $field_selected = isset($_POST['fields'][$i]) ? magic_quote_bypass($_POST['fields'][$i]) : '';

            $current_field = isset($_POST["field_$i"]) ? magic_quote_bypass($_POST["field_$i"]) : '';

            $order = $_POST["order_$i"] ?? '';

            $order_selected = $_POST["by_$i"] ?? '';

            $criteria = $_POST["crit_$i"] ?? '';
        ...
        $fields_list = mb_substr($fields_list, 0, -2);

        $like_list = mb_substr($like_list, 0, -4);

        $order_list = mb_substr($order_list, 0, -1);
